...
import {
  ...
  normalizeAttributeUnit,
  normalizeCoreColor,
  normalizeGearRarity,
  normalizeGearSlot,
  normalizeTalentType,
  normalizeWeaponClass,
  normalizeWeaponRarity,
  pickValue,
  readSheetRows,
  readSheetRowsText,
  safeCellText,
        ...
        for (const r of rows) {
          ...
          try {
            const data: any = {
              id,
              name: asString(r["name"]) ?? id,
              type: normalizeTalentType(asString(r["type"]) ?? "Weapon"),
        ...
        for (const r of rows) {
          ...
          try {
            const data: any = {
              id,
              name: asString(r["name"]) ?? id,
              category: normalizeAttributeCategory(asString(r["category"]) ?? "Offensive"),
              unit: normalizeAttributeUnit(asString(r["unit"]) ?? "PERCENT"),
              notes: asString(r["notes"]),
              sourceId: asString(r["source_id"]),
              lastUpdated: asDate(r["last_updated"]),
              patchVersion: asString(r["patch_version"]),
        ...
        for (const r of rows) {
          ...
          try {
            const data: any = {
              id,
              name: asString(r["name"]) ?? id,
              slot: normalizeGearSlot(asString(r["slot"]) ?? "Mask"),
              rarity: normalizeGearRarity(asString(r["rarity"]) ?? "HighEnd"),
              brandId: asString(r["brand_id"]),
              setId: asString(r["set_id"]),
              isNamed: asBool(r["is_named"]) ?? false,
              isExotic: asBool(r["is_exotic"]) ?? false,
              coreColor: normalizeCoreColor(asString(r["core_color"])),
        ...
        for (const r of rows) {
          ...
          try {
            const data: any = {
              id,
              name: asString(r["name"]) ?? id,
              class: normalizeWeaponClass(asString(r["class"]) ?? "AR"),
              rarity: normalizeWeaponRarity(asString(r["rarity"]) ?? "HighEnd"),
              isNamed: asBool(r["is_named"]) ?? false,
              isExotic: asBool(r["is_exotic"]) ?? false,
